...
#### *Velocity*  
Velocity is measured in meters/second. I convert velocity into speed or pace based on the commonly used metric in a given sport. I use seconds per 100 meters for swimming (sec/100m), kilometers per hour (km/h) for biking, and minutes per kilometer (min/km) for running. These are great for testing how well you are progressing towards your target paces or finish time. However, they have very limited uses outside of that. That is because velocity can be greatly impacted by factors outside your fitness. The number and steepness of hills on your course, the wind conditions, the number of traffic lights, and the terrain are all things outside your fitness that can impact your velocity. You can see that there is more randomness in the spread of the two graphs below compared to the graph above, showing that velocity is not the best indicator of fitness. That is why I only use it for exploratory analyses and to report race results.  
```{r, fig.show='hold', out.width='50%', message=FALSE, error=FALSE, warning=FALSE}
workouts %>%
  filter(WorkoutType == "Bike") %>%
